...
//   // Cool neutral rim for black torso highlights
//   const bodyRimLight = new THREE.PointLight(
//     0xd0e2f5,
//     2.8,
//     7.0,
...
//   // ----------------------------------------------------------
//   // Avatar container & Shoulder Shells (Matching Image 1)
//   // ----------------------------------------------------------
...
//   // High-gloss white ceramic/plastic armor for the shoulder caps
//   const whiteArmorMaterial = new THREE.MeshPhysicalMaterial({
//     color: 0xeeeeee,
//     roughness: 0.18,
//     metalness: 0.05,
//     clearcoat: 0.8,
...
//   // Glossy piano-black material for inner joints
//   const innerJointMaterial = new THREE.MeshPhysicalMaterial({
//     color: 0x050505,
//     roughness: 0.08,
//     metalness: 0.1,
//     clearcoat: 1.0,
...
//     // Dark glossy inner shoulder fixture
//     const lower = new THREE.Mesh(
//       new THREE.CylinderGeometry(
//         0.43,
//         0.50,
...
//       model.traverse((object) => {
//         if (object.name === "RoboFace_23") faceRoot = object;
//         if (object.name === "Jaw.R_19") jaw = object;
//         if (object.name === "Kito6_FaceMesh_Eye.L_20") leftEye = object;
//         if (object.name === "Kito6_FaceMesh_Eye.R_21") rightEye = object;
//         if (object.name === "Forehead_6") forehead = object;

//         if (!isMesh(object)) return;

//         // Clone/convert materials to Physical materials to accurately
//         // render the piano-lacquer deep black reflection of the first image
//         const origMaterials = Array.isArray(object.material)
//           ? object.material
//           : [object.material];

//         const updatedMaterials = origMaterials.map((mat) => {
//           const standard = mat as THREE.MeshStandardMaterial;
//           const r = standard.color ? standard.color.r : 1;
//           const g = standard.color ? standard.color.g : 1;
//           const b = standard.color ? standard.color.b : 1;
//           const luminance = 0.2126 * r + 0.7152 * g + 0.0722 * b;

//           const isDarkChestOrNeck = luminance < 0.22;

//           // Replace matte black with deep reflective lacquer
//           const physical = new THREE.MeshPhysicalMaterial({
//             map: standard.map,
//             normalMap: standard.normalMap,
//             roughnessMap: standard.roughnessMap,
//             metalnessMap: standard.metalnessMap,
//             color: isDarkChestOrNeck
//               ? new THREE.Color(0x06080b)
//               : standard.color.clone(),
//             roughness: isDarkChestOrNeck ? 0.05 : (standard.roughness ?? 0.35),
//             metalness: isDarkChestOrNeck ? 0.12 : (standard.metalness ?? 0.1),
//             clearcoat: isDarkChestOrNeck ? 1.0 : 0.0,
...
//     undefined,
//     (error) => {
//       console.error("[JARVIS AVATAR] Failed to load", MODEL_URL, error);
//     },
//   );
...
//     const next = customEvent.detail?.state;
...
//   let nextBlink = 2.0 + Math.random() * 3.0;
//   let blinkStart = -10;
//   let blinkActive = false;
//   let blinkDuration = 0.18;
...
//     avatar.position.y =
//       Math.sin(t * (speaking ? 1.55 : 0.65)) * (speaking ? 0.018 : 0.010);

//     aura.rotation.y = t * (thinking ? 0.20 : 0.10);
//     ring1.rotation.z = t * 0.13;
//     ring2.rotation.z = -t * 0.08;
//     ring3.rotation.z = t * 0.18;

//     particles.rotation.y = t * (thinking ? 0.035 : 0.018);

//     if (faceRoot) {
//       faceRoot.rotation.y =
//         Math.sin(t * 0.42) * (listening ? 0.026 : 0.012);
//       faceRoot.rotation.x = Math.sin(t * 0.31) * 0.008;
//     }
...
//       const scale =
//         1 + Math.sin(t * (speaking ? 6.0 : 2.8)) * pulse;
//       forehead.scale.setScalar(scale);
//     }

//     for (const eye of [leftEye, rightEye]) {
//       if (!eye) continue;
//       const base = eyeBase.get(eye);
//       if (!base) continue;

//       eye.position.copy(base.position);
//       eye.rotation.copy(base.rotation);
//       eye.scale.copy(base.scale);
//     }

//     if (leftEye && rightEye) {
//       const leftBase = eyeBase.get(leftEye);
//       const rightBase = eyeBase.get(rightEye);

//       if (leftBase && rightBase) {
//         if (t >= nextGazeChange) {
//           const horizontal = thinking
//             ? 0.028
//             : listening
//               ? 0.018
//               : 0.010;

//           const vertical = thinking
//             ? 0.014
//             : listening
//               ? 0.008
//               : 0.005;

//           targetGazeX = (Math.random() * 2 - 1) * horizontal;
//           targetGazeY = (Math.random() * 2 - 1) * vertical;

//           nextGazeChange =
//             t + (thinking ? 0.9 + Math.random() * 1.4 : 1.8 + Math.random() * 2.5);
//         }

//         gazeX = THREE.MathUtils.lerp(gazeX, targetGazeX, 0.055);
//         gazeY = THREE.MathUtils.lerp(gazeY, targetGazeY, 0.055);

//         leftEye.position.copy(leftBase.position);
//         rightEye.position.copy(rightBase.position);
...
//         const blinkFactor = blinkActive
//           ? 1 -
//             Math.sin(
//               THREE.MathUtils.clamp((t - blinkStart) / blinkDuration, 0, 1) *
//                 Math.PI,
//             ) *
//               0.96
//           : 1;

//         leftEye.scale.copy(leftBase.scale);
//         rightEye.scale.copy(rightBase.scale);
//         leftEye.scale.y *= blinkFactor;
//         rightEye.scale.y *= blinkFactor;

//         if (blinkActive && t - blinkStart >= blinkDuration) {
//           blinkActive = false;
//           nextBlink = t + 2.0 + Math.random() * 4.5;
//         }
//       }
//     }

//     if (jaw) {
//       const baseX = jawBaseRotationX;
//       if (speaking) {
//         const envelope =
//           0.50 +
//           0.30 * Math.sin(t * 7.1) +
...
//         const openAmount =
//           THREE.MathUtils.clamp(envelope, 0.04, 1.0) * 0.135;
...
//     bodyRimLight.intensity = speaking
//       ? 2.05
//       : thinking
//         ? 1.90
//         : executing
...
//     shoulderSpot.intensity = speaking
//       ? 2.25
//       : executing
//         ? 2.15
//         : 1.90;

//     bloom.strength = speaking
//       ? 0.24
//       : thinking
//         ? 0.20
//         : listening
